...
enum led_brightness_model {
	LED_BRIGHTNESS_MODEL_HARDWARE = 0,
	LED_BRIGHTNESS_MODEL_ONOFF,
	LED_BRIGHTNESS_MODEL_LINEAR,
	LED_BRIGTHNESS_MODEL_MAX
};
...
/**
 * led_common_setup_of - common device tree setup for led device
 *
 * @dev: the parent LED device for allocations
 * @led_cdev: the LED to update
 * @node: child node in device tree having settings
 *
 * Reads settings from device tree and configured color element based on it.
 *
 * Returns: 0 on success, or negative error value on failure.
 */
extern int led_common_setup_of(struct device *dev,
			       struct led_classdev *led_cdev,
